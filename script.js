...
const FALLBACK_MANIFEST = {
  tickets: [
    "A_B/tickets/Билет 1.json","A_B/tickets/Билет 2.json","A_B/tickets/Билет 3.json","A_B/tickets/Билет 4.json",
    "A_B/tickets/Билет 5.json","A_B/tickets/Билет 6.json","A_B/tickets/Билет 7.json","A_B/tickets/Билет 8.json",
    "A_B/tickets/Билет 9.json","A_B/tickets/Билет 10.json","A_B/tickets/Билет 11.json","A_B/tickets/Билет 12.json",
    "A_B/tickets/Билет 13.json","A_B/tickets/Билет 14.json","A_B/tickets/Билет 15.json","A_B/tickets/Билет 16.json",
    "A_B/tickets/Билет 17.json","A_B/tickets/Билет 18.json","A_B/tickets/Билет 19.json","A_B/tickets/Билет 20.json",
    "A_B/tickets/Билет 21.json","A_B/tickets/Билет 22.json","A_B/tickets/Билет 23.json","A_B/tickets/Билет 24.json",
    "A_B/tickets/Билет 25.json","A_B/tickets/Билет 26.json","A_B/tickets/Билет 27.json","A_B/tickets/Билет 28.json",
    "A_B/tickets/Билет 29.json","A_B/tickets/Билет 30.json","A_B/tickets/Билет 31.json","A_B/tickets/Билет 32.json",
    "A_B/tickets/Билет 33.json","A_B/tickets/Билет 34.json","A_B/tickets/Билет 35.json","A_B/tickets/Билет 36.json",
    "A_B/tickets/Билет 37.json","A_B/tickets/Билет 38.json","A_B/tickets/Билет 39.json","A_B/tickets/Билет 40.json"
  ]
};

const FALLBACK_QUESTION_BANK = [
  { ticket_number: "Демо билет 1", topic: "Общие положения",
    question: "Какой сигнал светофора разрешает движение?",
    answers: [{answer_text:"Зелёный",is_correct:true},{answer_text:"Жёлтый"},{answer_text:"Красный"},{answer_text:"Мигающий белый"}],
  ...
  { ticket_number: "Демо билет 1", topic: "Общие положения",
    ...
    answers: [{answer_text:"Всегда",is_correct:true},{answer_text:"Только ночью"},{answer_text:"Только когда нет других машин"},{answer_text:"Только в ясную погоду"}],
    tip:"На нерегулируемых переходах водитель обязан уступить дорогу пешеходам."},
  { ticket_number: "Демо билет 2", topic: "Начало движения, маневрирование",
    question: "Перед началом движения водитель должен",
    answers: [{answer_text:"Убедиться в безопасности манёвра",is_correct:true},{answer_text:"Подать звуковой сигнал"},{answer_text:"Включить аварийную сигнализацию"},{answer_text:"Выполнить обгон"}],
  ...
  { ticket_number: "Демо билет 2", topic: "Начало движения, маневрирование",
    ...
    answers: [{answer_text:"Подать соответствующий сигнал поворота",is_correct:true},{answer_text:"Снизить скорость до 20 км/ч"},{answer_text:"Включить дальний свет"},{answer_text:"Включить аварийную сигнализацию"}],
    tip:"Перед перестроением водитель обязан подать сигнал указателями поворота."},
  { ticket_number: "Демо билет 3", topic: "Скорость движения",
    question: "Какова максимальная разрешённая скорость в населённом пункте?",
    answers: [{answer_text:"60 км/ч",is_correct:true},{answer_text:"70 км/ч"},{answer_text:"80 км/ч"},{answer_text:"90 км/ч"}],
  ...
  { ticket_number: "Демо билет 3", topic: "Скорость движения",
    ...
    answers: [{answer_text:"150-300 метров",is_correct:true},{answer_text:"50-100 метров"},{answer_text:"400-500 метров"},{answer_text:"Не ближе 20 метров"}],
...
======================= */
function initApp(){
  try { bindMenu(); bindDelegation(); } catch(err){ console.error("Ошибка инициализации интерфейса:", err); }
  boot();
}

if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", initApp, { once: true });
} else { setTimeout(initApp, 0); }
...
function setLoader(p){ const bar = qs("#loaderBar"); if(bar) bar.style.width = Math.max(0,Math.min(100,p))+"%"; }
...
function handleTap(e){
  ...
  if (e.target.closest("[data-prev]")){ e.preventDefault(); previousQuestion(); return; }
  if (e.target.closest("[data-next]")){ e.preventDefault(); nextQuestion(); return; }
  if (e.target.closest("[data-finish]")){ e.preventDefault(); finishDuel(); return; }
...
function handlePointerUp(e){
  ...
  if (!tap.moved && tap.target) {
    ...
    if (finalTarget === tap.target) {
      const synthetic = { type: "pointerup", pointerType: "touch", target: finalTarget, preventDefault: () => e.preventDefault() };
      handleTap(synthetic);
...
function handlePointerCancel(){ State.tap = null; }
function handleClick(e){
  if (State.ignoreClickUntil && Date.now() < State.ignoreClickUntil) { return; }
  handleTap(e);
}
...
async function loadTickets(onProgress){
  ...
  catch(err){ console.warn("⚠️ Не удалось загрузить manifest, используем запасной список", err); }
  const ticketFiles = uniqueStrings([ ...(manifest?.tickets || []), ...FALLBACK_MANIFEST.tickets ]);
  ...
  const raw = [];
  let loaded = 0, successes = 0, failures = 0;
...
async function loadPenalties(){
  ...
  for(const line of lines){
    try {
      ...
      items.push({ articlePart: obj.article_part || obj.articlePart || "—", text: obj.text || "", penalty: obj.penalty || "" });
    } catch(err){ console.error("Не удалось разобрать штраф:", err, line); }
  }
...
function uiTickets(){
  const tickets = [...State.byTicket.entries()].map(([key, meta]) => ({
    key, label: meta.label || key, order: Number.isFinite(meta.order) ? meta.order : Number.MAX_SAFE_INTEGER, questions: meta.questions
...
function uiStats(){ setView(`<div class="card"><h3>Статистика</h3><p>Скоро здесь будет прогресс дуэлей.</p></div>`, { subpage: true, title: "Статистика" }); }
...
function startDuel({mode,topic=null}){
  ...
  State.duel = { mode, topic, i:0, me:0, q, answers: Array(q.length).fill(null), furthest: 0, completed: false };
  renderQuestion(0);
}
function startTicket(key){
  ...
  State.duel = { mode:"ticket", topic:null, i:0, me:0, q, ticketLabel: bucket?.label || key, answers: Array(q.length).fill(null), furthest: 0, completed: false };
  renderQuestion(0);
}
function renderQuestion(targetIndex){
  ...
  if(targetIndex >= d.q.length){ finishDuel(); return; }
...
function onAnswer(i){
  ...
  if(prev?.status){ State.lock = false; return; }
  const isCorrect = (i === correct);
  if(isCorrect) d.me++;
  d.answers[d.i] = { status: isCorrect ? "correct" : "wrong", selected: i };
  d.furthest = Math.min(d.q.length - 1, Math.max(d.furthest, d.i + 1));
  if(isCorrect){ toast("✅ Верно!"); } else { toast("❌ Ошибка"); }
  renderQuestion(d.i);
  if(isCorrect){
    State.advanceTimer = setTimeout(()=>{
      if(State.duel === d && d.i === currentIndex && d.answers[currentIndex]?.status === "correct"){ nextQuestion(); }
    }, 650);
  }
}
function finishDuel(){
...
function updateStatsCounters(){ setStat("statQuestions", State.pool.length); setStat("statTopics", State.topics.size); setStat("statTickets", State.byTicket.size); }
function setStat(id, value){ const el = qs(`#\${id}`); if(!el) return; el.textContent = value ? value.toLocaleString("ru-RU") : "0"; }
function formatNumber(value){ return Number.isFinite(value) ? value.toLocaleString("ru-RU") : "0"; }
function clearAdvanceTimer(){ if(State.advanceTimer){ clearTimeout(State.advanceTimer); State.advanceTimer = null; } }
function notifyDataIssue(){ if (State.pool.length) return; toast("⚠️ Не удалось загрузить билеты. Проверьте соединение и обновите страницу."); }
...
function renderAnswerButton(text, index, question, answerState){
  const classes = ["answer"]; let disabled = "";
...
function renderQuestionControls(isAnswered){
  ...
  return `<div class="question-controls">\${prevBtn}\${nextBtn}</div>`;
}
...
function nextQuestion(){
  const d = State.duel; if(!d) return;
  clearAdvanceTimer();
  if(d.i >= d.q.length - 1){ if(d.answers[d.i]?.status){ finishDuel(); } return; }
